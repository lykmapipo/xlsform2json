...
function type(question) {
    ...
    if (!_.isEmpty(question.type) && (question.type.match(/select_one/g) ||
...
function validate(question) {
    ...
    //ensure lowercase and snake case style variable name
    if (question && !_.isEmpty(question.name) &&
        _.snakeCase(question.name) !== question.name) {
        throw new Error([
            'Invalid variable name ',
            '(', question.name, ')',
            ' on survey sheet at row number ',
            question.row,
            '. Use lowercase letters and replace space with underscore(_) when naming variable.'
        ].join(''));
    }
...
module.exports = exports = function(questionnaire, survey, done) {
    try {
        ...
        var duplicates = _.cloneDeep(survey);
        duplicates = _.groupBy(duplicates, 'name');
        _.forEach(duplicates, function(duplicate, name) {
